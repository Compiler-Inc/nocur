...
impl ClaudeSession {
    ...
    pub fn new_with_config(working_dir: &str, app_handle: AppHandle, config: ClaudeSessionConfig) -> Result<Self, String> {
        ...
        let project_id = crate::ace::generate_project_id(working_dir);
        log::info!("ACE project ID: {}", project_id);
...
fn parse_service_event(json: &serde_json::Value, raw_line: &str) -> Option<ClaudeEvent> {
    ...
    match event_type.as_str() {
        ...
        "ace_bullets_used" => {
            // ACE: Bullets that were used in the response
            let bullets_used = json.get("bulletsUsed")
                .and_then(|b| b.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect());
            let bullets_included = json.get("bulletsIncluded")
                .and_then(|b| b.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect());
        ...
        "ace_task_complete" => {
            // ACE: Task completion for potential reflection
            let outcome = json.get("outcome")
                .and_then(|o| o.as_str())
                .map(String::from);
        ...
        "lsp_progress" => {
            // LSP: Progress updates during language server initialization
            let message = json.get("message")
                .and_then(|m| m.as_str())
                .unwrap_or("")
                .to_string();
